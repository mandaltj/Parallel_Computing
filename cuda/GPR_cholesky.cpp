...
void Cholesky_factorization(Matrix & L, Matrix & U){
    ...
    for (int j = 0; j < dimension; j++) {
        float sum = 0;
        for (int k = 0; k < j; k++) {
            sum += L[j*dimension+k] * L[j*dimension+k];
            //fops += 2;
        }
        L[j*dimension+j] = sqrt(U[j*dimension+j] - sum);
        //fops += 2;
		...
		//fops += 2;
            ...
            L[i*dimension+j] = (1.0 / L[j*dimension+j] * (U[i*dimension+j] - sum));
	    //fops += 2;
        }
    }
